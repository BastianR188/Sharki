class FinalEnemy extends MoveableObject {
  ...
  Animate() {
    ...
    setInterval(() => {
      try {
        if (this.live <= 0) {
          world.statusbar.statusbarFinalEnemy[0].img.src =
            "./img/4. Marcadores/orange/0_  copia.png";
    ...
    let intervalId = setInterval(() => {
      ...
      if (this.StartIntro && !this.isAnimating && !this.spamProtection) {
        if (this.isDead()) {
          ...
          this.FinalEnemyHitSound.volume =
  ...
  MoveAI() {
    if (!this.attacking) {
      // Wenn der Gegner nicht angreift
      this.y += this.enemyDirection * this.speed; // Bewege den Gegner in der aktuellen Richtung mit der aktuellen Geschwindigkeit
      if (this.y + this.coly < 0) {
        // Wenn der Gegner das obere oder untere Ende des Canvas erreicht hat
      ...
      if (this.attackTimer >= Math.floor(Math.random() * 85) + 160) {
        // Wenn der Timer abgelaufen ist (5-10 Sekunden)
      ...
      if (this.attackTimer == this.attackDistance) {
        // Wenn der Gegner die Angriffsstrecke erreicht hat
        this.attackTimer = 0;

class MoveableObject extends DrawAbleObject {
  ...
  isNearby(mo) {
    return (this.distanceBetween = Math.sqrt(
      (this.x +
        this.colx +
        this.colwidth / 2 -
        (mo.x + mo.colx + mo.colwidth / 2)) **
        2 +
        (this.y +
          this.coly +
          this.colheight / 2 -
          (mo.y + mo.coly + mo.colheight / 2)) **
          2
    ));
  }
  ...
  isColliding(mo, val) {
    return (
      this.x + this.colx + this.colwidth + val > mo.x + mo.colx &&
  ...
  createBubble(img, od, p) {
    ...
    if (od) {
      ...
      bubble = new Bubble(
        img,
        this.x + this.colx + this.colwidth,
    ...
    let intervalIdmoveToCharY = setInterval(() => {
      ...
      try {
        if (!this.isDead() && !this.isHurt() && world.character.live > 0) {
          if (
            this.SwimLine >
            world.character.y + world.character.coly + world.character.colheight
          ) {
      ...
      try {
        if (!this.isDead() && !this.isHurt() && world.character.live > 0) {
          if (
            this.x >=
            world.character.x +
              world.character.colx +
              world.character.colwidth / 2
          ) {
            this.x -= this.speed;
